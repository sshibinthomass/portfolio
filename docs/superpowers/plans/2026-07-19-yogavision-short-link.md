...
**Goal:** Make `/yogavision` redirect to `/en/projects/3` using the portfolio's existing client-side short-link pattern.
...
- Use the correct spelling `/yogavision`.
...
**Files:**
- Create: `tests/yogavision-redirect.test.mjs`
- Modify: `src/App.jsx`

**Interfaces:**
- Consumes: React Router's existing `Navigate` component and the English short-URL route block.
- Produces: A `/yogavision` route whose element navigates to `/en/projects/3` with `replace` enabled.
...
Run: `node --test tests/yogavision-redirect.test.mjs`

Expected: FAIL because `src/App.jsx` does not yet contain the `/yogavision` route and still contains the misspelled alias.
...
```powershell
node --test tests/yogavision-redirect.test.mjs
npm test
npm run lint
npm run build
```
...
```powershell
git add -- src/App.jsx tests/yogavision-redirect.test.mjs docs/superpowers/plans/2026-07-19-yogavision-short-link.md
